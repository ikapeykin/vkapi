...
class VkApi:
    ...
    def __init__(self, username='', password='', *, ignored_errors=None, timeout=5, log_file='', captcha_handler=None, token_file='', token=None, group_id=''):
    ...
    def initLongpoll(self):
        r = self.groups.getLongPollServer(group_id=self.group_id)
        if not r:
            logger.warning('Unable to initialize longpoll')
            self.longpoll = {}
            return
        self.longpoll = {'server': r['server'], 'key': r['key'], 'ts':r['ts']}
